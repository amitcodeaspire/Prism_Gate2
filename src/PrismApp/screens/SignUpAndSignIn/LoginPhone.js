    ...
    if (userAuth) {
      ...
      try {
        const url =
          ...
          console.log(response.data.flatno);
          setUser(userAuth);
          authCtx.setuid(uid);
          ...
          // return Alert.alert(
          //   'User Not Found',
          //   'This User is Not Register Please Register it first',
          // );
        }
      setLoading(false);
    }
  ...
  const signInWithMobileNumber = async () => {
    ...
    if (ShowCaseMobile.length === 10) {
      try {
        ...
        Alert.alert(
          'Failed',
          'This Number is alreday register or Please Check OTP or Or invalid number' + error,
        );
      }
    } else {
      Alert.alert(
        ...
        {confirm === null && (
          <>
            ...
            <View
              ...
              />
              {/* <TouchableOpacity
                onPress={() => navigation.replace('SignUpPhone')}
                style={{
                  marginTop: 20,
                  marginBottom: -10,
                }}>
